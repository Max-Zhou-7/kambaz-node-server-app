...
export default function WorkingWithArraysAsynchronously() {
  ...
  const updateTodo = async (todo: any) => {
    ...
    setTodos(todos.map((t) => (t.id ===todo.id ? todo : t)));
        ...
        setErrorMessage(error.response.date.message);
    }
  };
  ...
  return (
    <div id="wd-asynchronous-arrays">
      ...
      <h4>Todos <FaPlusCircle onClick={createNewTodo} className="text-success float-end fs-3"/> 
                <FaPlusCircle onAbort={postNewTodo} className="text-primary float-end fs-3 me-3" id="wd-post-todo" />
      ...
      <ListGroup>
        {todos.map((todo) => (
          <ListGroupItem key={todo.id}>
            ...
            <TiDelete onClick={() => deleteTodo(todo)} className="text-danger float-end me-2 fs-3" id="wd-deleted-todo" />
